...
export default function CupomTeste({ pedido, onClose, onFinalizar }) {
  if (!pedido) {
  ...
  const total = Number(
    pedido.valorTotal || pedido.totalComJuros || pedido.total || 0,
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white p-4 rounded shadow max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div style={{ position: "sticky", bottom: 0, background: "#fff" }}>
          ...
          >
            ...
            {pedido.itens?.map((i, idx) => {
              const descricao = i.produto?.title || i.descricao || "Produto";
            ...
            {pedido.pixCodigo && (
              <>
                ...
                <div style={{ wordBreak: "break-all" }}>{pedido.pixCodigo}</div>
              </>
            )}
